...
def _():
    ...
    from bblean_cluster import bblean_cluster    # clustering large datasets
...
def _(mo):
    ...
    We see a couple of messages below the table indicating that Marimo doesn't know how to handle the data in that column. I need to find a way around this.
    """)
    return
...
def _(mo):
    mo.md("""
    r"""Create a list called `assay_cols` containing the names of the log transformed assay columns in `train_df`.""
    """)
    return
...
def _(mo):
    mo.md("""
    r"""To make it easy to select the clusters with more than 10 members, we'll put the cluster ids, and corresponding cluster sizes, into a dictionary.""
    """)
    return
...
def _(mo):
    mo.md("""
    r"""Here we calcuate the principal components from the "fp" column of the training dataframe.""
    """)
    return
...
def _(mo):
    mo.md("""
    r"""Next we create a new dataframe, `big_cluster_df`, containing only the clusters with 10 or more members. This will allow us to focus our analysis on the larger clusters, which are likely to be more informative. We then display the dataframe, excluding the fingerprint columns for clarity.""
    """)
    return
...
def _(Chem, label_list, mol_list, uru):
    ...
    Chem.Draw.MolsToGridImage(mol_list,molsPerRow=4,subImgSize=(300,300),legends=label_list)
    return
